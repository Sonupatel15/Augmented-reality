...
# st.title("AR Wedding Card Experience")
...
# Import AR backend functionality
from ar_core.ar_main import process_frame
from ar_core.camera import initialize_camera, release_camera
from overlay_modules.object_loader import OBJ
from utils.projection import projection_matrix
...
if overlay_type == "3D":
    ...
    if os.path.exists("assets/models"):
        ...
        model_options.extend(model_files)
    model_selection = st.sidebar.selectbox("Select 3D model", model_options)
    st.session_state.model_path = os.path.join("assets/models", model_selection if model_selection != "Default (Fox)" else "fox.obj")
...
st.sidebar.subheader("📷 Camera Control")
camera_button_label = "Stop Camera" if st.session_state.camera_active else "Start Camera"
if st.sidebar.button(camera_button_label):
    st.session_state.camera_active = not st.session_state.camera_active
    if not st.session_state.camera_active:
        st.rerun()

if st.session_state.camera_active:
    if st.sidebar.button("📸 Capture Moment"):
        st.session_state.capture_requested = True
...
def run_ar_camera():
    ...
    obj = OBJ(st.session_state.model_path, swapyz=True) if st.session_state.overlay_type == "3D" else None
    image_overlay = cv2.imread(st.session_state.image_path) if st.session_state.overlay_type == "IMAGE" else None
    video_cap = cv2.VideoCapture(st.session_state.video_path) if st.session_state.overlay_type == "VIDEO" else None
    ...
    while st.session_state.camera_active:
        ...
        try:
            frame = process_frame(frame, reference_image, np.eye(3), obj, image_overlay, video_cap, st.session_state.overlay_type, st.session_state.min_matches)
            if frame is None:
                status_text.text("Not enough matches")
                continue
        except Exception as e:
